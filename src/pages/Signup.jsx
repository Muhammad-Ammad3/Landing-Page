      ...
      <div className="relative w-full max-w-md bg-white p-8 rounded-3xl shadow-xl border border-slate-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {[
            ...
            { name: 'password', label: 'Password', type: showPassword ? 'text' : 'password', icon: FaLock },
            { name: 'confirmPassword', label: 'Confirm Password', type: showPassword ? 'text' : 'password', icon: FaLock }
          ].map((field) => (
            <div key={field.name}>
          ...
          <button disabled={isLoading} className="w-full py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition flex justify-center">
